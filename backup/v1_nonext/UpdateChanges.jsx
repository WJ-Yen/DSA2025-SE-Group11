// import {getN, ConstructTree} from "../../../backend"

// update the value of a node in the tree
function updateNodeValue(node, targetIndex, newValue) {
	if (parseInt(node.name) === targetIndex) {
		// console.log("update node", node.name, ": ", newValue);
		node.value = newValue;
...
function resetNodeColor(node, targetIndex) {
	if (parseInt(node.name) === targetIndex) {
	  node.color = "#ffffff";
	  return true;
	}
...
export default function UpdateChanges(tree, changes, setTreeData) {
	const newTree = structuredClone(tree); // Make a deep copy
	
	// Instead of calling applyUpdates directly, we'll schedule the updates
